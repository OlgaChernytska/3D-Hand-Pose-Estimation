...
'Joint Regression Network (JORNet) implementation from Paper https://arxiv.org/pdf/1704.02201.pdf'
...
class JORNet(nn.Module): # input size 128x128
    def __init__(self, n_keypoints=21):
        ...
        self.res_block3_conv = ResidualBlockConvolution(512, s=2, f1=256, f2=1024)
        
        self.res_block3_identities = nn.Sequential(
        ...
        self.heatmaps_inter = nn.Sequential(
            nn.ConvTranspose2d(1024, 256, kernel_size=(3,3), stride=2, bias=False),
            nn.ConvTranspose2d(256, 64, kernel_size=(3,3), stride=2, bias=False),
            nn.ConvTranspose2d(64, 21, kernel_size=(4,4), stride=2, bias=False),
            nn.Sigmoid()
        )
        
        self.heatmaps_final = nn.Sequential(
            nn.ConvTranspose2d(256, 128, kernel_size=(3,3), stride=2, bias=False),
            nn.ConvTranspose2d(128, 64, kernel_size=(3,3), stride=2, bias=False),
            nn.ConvTranspose2d(64, 21, kernel_size=(4,4), stride=2, bias=False),
            nn.Sigmoid()
        )
        ...
        self.keypoints_3d_prediction = nn.Sequential(
            nn.Linear(256 * 15**2, 200, bias=True),  # dimensionality is hard coded
            nn.ReLU(inplace=True),
    ...
    def forward(self, sample):
        ...
        inter_features2 = self.res_block3_conv(features)
        features = self.res_block3_identities(inter_features2)
        features = self.conv_block(features)
        
        heatmaps_inter = self.heatmaps_inter(inter_features2)
        heatmaps_final = self.heatmaps_final(features)
        
        out_3d = self.keypoints_3d_prediction(features.view(features.size(0), -1))
        
        return {'heatmaps_inter': heatmaps_inter,
                'heatmaps': heatmaps_final,
                'vector_3d': out_3d }
